...
RUNTIME_ROOT = Path(os.getenv("CORRECTION_RUNTIME_ROOT", str(BASE_DIR))).resolve()
LOGS_DIR = RUNTIME_ROOT / "logs"
STATE_DIR = RUNTIME_ROOT / "state"
REPORTS_DIR = RUNTIME_ROOT / "reports" / "paper_trading"
ENV_PATH = Path(os.getenv("CORRECTION_ENV_PATH", str(BASE_DIR / ".env"))).resolve()
...
def load_paper_state(path: Path, initial_capital: float) -> Dict[str, Any]:
    ...
    if not payload["closed_trades"] and payload["open_trade"] is None:
        configured_capital = float(initial_capital)
        current_capital = float(payload.get("current_capital", configured_capital))
        previous_initial = float(payload.get("initial_capital", configured_capital))
        if abs(previous_initial - configured_capital) > 1e-9 and abs(current_capital - previous_initial) <= 1e-9:
            payload["initial_capital"] = configured_capital
            payload["current_capital"] = configured_capital
    return payload
        ...
        if now_ts - last_heartbeat >= args.heartbeat_minutes * 60:
            ...
            print_status(
                "heartbeat",
                {
                    ...
                    "symbol": args.symbol,
                    "paper_mode": paper_mode,
                    "capital": paper_summary(paper_state)["current_capital"] if paper_mode else args.initial_capital,
